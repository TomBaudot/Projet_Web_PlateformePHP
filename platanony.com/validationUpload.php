<?php   define('ROOT_PATH', realpath(dirname(__FILE__)));?>
    ...
    <?php
    define("UPLOAD_REP", __DIR__ ."/uploads/");
    define("UPLOAD_SIZEMAX", 1000000000000000); // La taille, en octets.
    define("UPLOAD_EXTENSION", "csv,txt");
    ...
    if (!empty($_POST)) {
        ...
        if ($_FILES['soumission']['error'] != 0)
        {
            ...
            if ($_FILES['soumission']['size'] > UPLOAD_SIZEMAX) {
                ...
                header("Location:index.php?controller=soumission&action=print_error&error=size");
            }
            ...
            if (!in_array($file_Extension, explode(',', constant('UPLOAD_EXTENSION'))))
            {
                ...
                header("Location:index.php?controller=soumission&action=print_error&error=extension");
            }

            // On vérifie le type mime
            if ($file_MimeType != UPLOAD_MIMETYPE)
            {
                //echo("mime:".$file_MimeType." alors que :".UPLOAD_MIMETYPE);
                header("Location:index.php?controller=soumission&action=print_error&error=mime");
            ...
            if (empty($errors))
            {
                ...
                if(!file_exists(UPLOAD_REP . $key)){
                  ...
                  if (move_uploaded_file($_FILES['soumission']['tmp_name'], UPLOAD_REP . $key)) {
                      ...
                      $_SESSION['name_file'] = $_FILES['soumission']['name'];

                      $_SESSION['flash']['success'] = 'Upload effectué avec succès !';

                      header("Location:index.php?controller=soumission&action=upload&id=0&nom_fichier=".$key."&id_team=0");

                  } else {
                      header("Location:index.php?controller=soumission&action=print_error&error=unknown");
                  ...
                  header("Location:index.php?controller=soumission&action=print_error&error=file_exists");
                }
            }
            else {
